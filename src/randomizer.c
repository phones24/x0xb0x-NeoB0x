...
uint16_t scaledNotes[7] = {
	// bit wise encoded version
	/*111111111111*/ 0x0fff, // chromatic
	/*010010101001*/ 0x04a9, // minor pentatonic
	/*010110101101*/ 0x05ad, // natural minor
	/*101010110101*/ 0x0ab5, // major
	/*010110101011*/ 0x05ab, // phrygian
	/*010101101011*/ 0x056b, // locrian
	/*011010110101*/ 0x06b5  // mixolydian
};
...
{
	uint8_t scale = rndScale;
	uint8_t setup;

	if (setupMode == REUSE_SETUP)
	{
	...
	uint8_t multiProp =
		randSetups[setup]
				  [4]; // probability of step is joined with next(s) to build a longer note/rest..
	...
	for (uint8_t i = 0; i < p_len; /*noop*/)
	{
		...
		if (useStructuredRests &&
			((randomIf(15)) || (i > 11 && randomIf(30)))) // sometimes break the rules
		...
		else
		{
			...
			if (!waupCount && key != lastKey &&
				lastKey > 0) // only for changed pitch, bindings are handled elsewhere
			{
...
{
	uint8_t scale = rndScale; // reroll in the currently selected scale
	if (scale >= 7)
		scale = 0;
